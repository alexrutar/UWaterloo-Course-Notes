...
\begin{proof}
    ...
    \end{itemize}
    Consider the set of all finite minimal cuts in $G_{i,1,1}$, and for each $i$, let $D_{1,i,1},D_{2,i,1},D_{3,i,1},\ldots$ be the cuts contained in $G_{i,1,1}$.
    Since each cut is finite, there are only countably many such $D_{k,i,1}$.

    Consider the cut $D_{1,i,1}$ in $G_{i,1,1}$.
    If this is not a minimal cut in $G$, then it is not a minimal separating set in $G$, and hence not a separating set in $G$.
    Thus there is some path $p_i$ in $G$ not contained in $G_{i,1,1}$.
    Place 2 vertices on each edge in $D_{1,i,1}$, contract the first vertices into some $v_1$, and the second vertices into $v_2$, and remove the edges between $v_1$ and $v_2$ except for 1 edge $e_i$; call this graph $G'_{i,1,1}$ and set $G'=\bigcup_{i\in I(1,1)}G'_{i,1,1}$.
    This new graph is not bridgeless since there is a path in $G$ not contained in $G_{i,1,1}$.
    Again, we may cover $G'$ by cycles $\{C_\alpha:\alpha\in A'\}$ using the previous proposition and define the graph structure on $A'$ as before.
    Let $C_\beta$ be the cycle in $G'$ containing $e_i$.
    Let $\{A'_i:i\in I(1,2)\}$ be the components of $A'$ and set $G_{i,1,2}=\bigcup_{\alpha\in A_i'}C_\alpha$.
