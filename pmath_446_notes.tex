...
\end{proof}
\begin{lemma}
    Given an $A-$module $P$ an an $A-$linear map $f:M\to N$, there is a unique $A-$linear map
    \begin{equation*}
...
\end{remark}
\begin{proposition}
    $N\otimes_A\bigcdot$ is right exact: i.e. if $M'\to M\to M''\to 0$ is exact, then $N\otimes M'\fto{1\otimes f}N\otimes M\fto{1\otimes g}N\otimes M''\to 0$ is exact.
\end{proposition}
\begin{proof}
...
\end{definition}
\begin{proposition}\label{ex-ten}
...
\begin{proof}
    ...
    $(ii\Leftrightarrow iii)$ follows by \cref{ex-ten}.

    $(iii\Rightarrow iv)$ is obvious.

    $(iv\Rightarrow iii)$.
    Let $:M'\to M$ be injective and let $u=\sum x_i\otimes y_i\in\ker(f\otimes 1)$.
    In other words,
\end{proof}
Suppose $f:M'\to M$ is injective.
Then $0\overset{f}{\to} M'\overset{\pi}{\to} M/\im(f)\to 0$ is exact.
By (ii), $0\to N\otimes M'\overset{1\otimes f}{\to}N\otimes M\overset{1\otimes \pi}{\to}N\otimes M/\im(N)\to 0$ is exact, so $1\otimes f$ is injective.
\begin{example}
    Free modules are flat.
\end{example}
\begin{proof}
    Suppose $f:M'\to M$ is injective, $F=\oplus_{i\in I}A$ a free $A-$module.
    We want to show $F\otimes M'\to F\otimes M$ is injective.
    Since tensors commute with direct sum, $F\otimes M'=\oplus_{i\in I}(A\otimes_A M')=\oplus_{i\in I}M'$, we have $1\otimes f:\oplus_{i\in I} M'\to\oplus_{i\in I}M$.
    But then $1\otimes f(x_i:i\in I)=(f(x_i):i\in I)$.
\end{proof}
\section{Algebras}
\begin{definition}
    An \textbf{A-Algebra} is a ring $B$ (commutative unitary) equipped with a ring homomorphism $f:A\to B$.
\end{definition}
Note that this endows $B$ with an $A-$module structure: $a\in A$, $b\in B$, then $ab:=f(a)b$.
Then $f:A\to B$ is a homomorphism of $A-$modules: if $a\in A$, $x\in A$, then $f(ax)=f(a)f(x)=af(x)$.
This $A-$module structure on $B$ satisfies compatibility with $\times$ on $B$: $(*)$ $a(b_1b_2)=(ab_1)b_2$
\begin{lemma}
    Suppose $B$ is a ring with an $A-$module structure satisfying $(*)$.
    Then there is a unique ring homomorphism $f:A\to B$ inducing this module structure on $B$.
\end{lemma}
\begin{proof}
    Define $f:A\to B$ by $f(a)=a1_B$.
    We just check multiplicativity: $f(a_1a_2)=a_1a_21_b=a_1(a_21_B)=a_1((1_B)(a_21_B))=(a_11_B)(a_21_B)=f(a_1)f(a_2)$ by $(*)$.
\end{proof}
Thus $A-$algebras are given by $A-$Modules with ring structure satisfying $(*)$.
\begin{example}
    Suppose $A=k$ is a field.
    Then a $k-$algebra is a ring extending $k$ with its $k-$vector space structure.
    A ring homomorphism $f:k\to B$ must be injective, so we identify $k$ with its image in $B$.
\end{example}
\begin{example}
...
\begin{definition}
    If $(B,f)$, $(C,g)$ are $A-$algebras, then a \textbf{homomorphism of $A-$algebras} is a ring homomorphism $\phi:B\to C$ such that $\phi(f(a))=g(a)$ for all $a\in A$.
    These are also called \textbf{A-linear (ring) homomorphisms}.
\end{definition}
\begin{definition}
    Let $(B,f)$ be an $A-$algebra.
    Then a \textbf{sub-A-algebra} is a subring $C\subseteq B$ containing $f(A)$.
    Then \textbf{A-subalgebra generated by $x$}, denoted by $A[x]$ is the smallest subalgebra of $B$ containing $x$.
\end{definition}
...
\begin{definition}
    $(B,f)$ is a \textbf{finite $A-$algebra} if it is finitely generated as an $A-$module.
\end{definition}
\begin{example}
...
\end{proposition}
\begin{proof}
    $B$ is a finite $A-$algebra, $f:A\to B$.
    Let $b_1,\ldots,b_n\in B$ which generate $B$ as an $A-$module.
    Consider $A[b_1,\ldots,b_n]$ be the $A-$subalgebra of $B$ generated by $\{b_1,\ldots,b_n\}$.
    Let $b\in B$, then $b=a_1b_1+\cdots+a_nb_n=P(b_1,\ldots,b_n)$ for some $a_1,\ldots,a_n\in A$.
    Since each $a_ib_i\in A[b_1,\ldots,b_n]$, so $b\in A[b_1,\ldots,b_n]$ and $B=A[b_1,\ldots,b_n]$.
\end{proof}
\begin{example}
    $\Q[x]/(t^2)$ is a finite $\Q-$algebra since
    \begin{align*}
        (a_nt^n+\cdots+a_1t+a_0)+(t^2)&=t^2(a_nt^{n-2}+\cdots+a_3t+a_2)+a_1t+a_0+I\\
                                      &= a_1t+a_0+I\\
                                      &= a_1(t+I)+a_0(1+I)
    \end{align*}
    so $B=\spn_{\Q}(t+I,1+I)$.
\end{example}
\begin{remark}
    If $B$ is an $A-$algebra and $I\subseteq B$ an ideal, then $B/I$ has a canonical $A-$algebra structure $A\fto{f} B\fto{\pi} B/I$.
\end{remark}
\begin{lemma}
    If $(B,f)$ is a finitely generated $A-$algebra, then $B\cong A[t_1,\ldots,t_n]/I$ where $t_1,\ldots,t_n$ are indeterminants and $I\subseteq A[t_1,\ldots,t_n]$ an ideal.
\end{lemma}
\begin{proof}
    ...
    Let $I=\ker(\phi)$.
    By the 1st Isomorphism Theorem for rings, $A[t_1,\ldots,t_n]/I\cong B$ as rings, the isomorphism $P(t_1,\ldots,t_n)+I\mapsto P(b_1,\ldots,b_n)$ is $A-$linear.
\end{proof}
Let $(B,f)$ denote an $A-$algebra, $M$ an $A-$module.
Since $(B,f)$ is also a module, we can consider $M_B=B\otimes_A M$ as an $A-$module.
Then $M_B$ has a natural $B-$module structure given by $b\in B_j$, $\sum_i b_i\otimes x_i$, $b_i\in B$, $x_i\in M$.
Then
\begin{align*}
    b\cdot\left(\sum_i b_i\otimes x_i\right):=\sum_i bb_i\otimes x_i
\end{align*}
Thus makes $M_B$ a $B-$module.
We call it the extension by scalars of $M$.

We can also go the other way: suppose $N$ is a $B-$module.
We can consider it as an $A-$module via $ax:=f(a)x$.
This $A-$module is called the restriction of scalars of $N$.
...
\end{itemize}
Note that $B\otimes_A C$ also has an $A-$algebra structure $A\to B\otimes_A C$ by $a\mapsto f(a)\otimes 1=1\otimes g(a)$.
Then the following diagram commutes:
\begin{center}
    \begin{tikzcd}
        A\arrow[r, "f"]\arrow[rd,"f"]& B \arrow[r]& B\otimes_A C\arrow[d,"\id"]\\
                                    & C \arrow[r]& B\otimes_A 
    \end{tikzcd}
\end{center}
\chapter{Fractions}
\begin{definition}
    Let $A$ be a ring.
    Then we say $S\subseteq A$ is \textbf{multiplicitly closed} if $s,t\in S$ implies $st\in S$ and $1\in S$.
\end{definition}
...
We denote by $S^{-1}A:=A\times S/\equiv$.
We make this into a ring by $0=0/1$ and $1=1/1$.
\begin{equation*}
    \frac{a}{s}+\frac{b}{t}=\frac{at+bs}{st},\quad\frac{a}{s}\frac{b}{t}=\frac{ab}{st}
\end{equation*}
One can verify that this is well-defined and defines a commutative unitary ring structure on $S^{-1}A$, such that the map $A\to S^{-1}A$ by $a\mapsto a/1$ is a ring homomorphism (not necessarily injective).
We call this the \textbf{ring of fractions of $A$ with respect to $S$}.
In fact, $S^{-1}A$ is an $A-$algebra via the map $A\to S^{-1}A$ by $a\mapsto a/1$.
\begin{remark}
    ...
    In this case, $a\mapsto a/1$ is actually injective.

    In general, $\ker(A\to S^{-1}A)=\{a\in A:\exists v\in S:av=0\}$.
\end{remark}
\begin{remark}
...
\begin{example}
    ...
    We denote $A_P:=S^{-1}A$ and call it the \textbf{localisation of $A$ at $P$}.
    Note that $A_P$ is a local ring (there is a unique maximal ideal).
    To see this, consider the ideal in $A_P$ generated by $\{a/1:a\in P\}$, and call it $PA_P$.
    (In general, if $f:A\to B$ is an $A-$algebra and $I\subseteq A$ an ideal, then $IB:=\{f(a):a\in I\}$ is an extension ideal.)
    One can verify that $PA_P=\{a/s:a\in P,s\notin P\}$.
    If $\frac{a}{s}\in A_P\setminus PA_P$, then $a\notin P$ so $a\in S$, hence $\frac{s}{a}\in A_P$, so $\frac{s}{a}\in _P$.
    ...
    That is to say, every proper ideal of $A_p$ is contained in $PA_P$.
    Thus $A_P$ is a local ring with a unique maximal ideal $PA_P$.

    Consider $A=\Z$, $p$ a prime number, $P=(p)$.
    Then $\Z_{(p)}=\left\{\frac{r}{s}:r,s\in\Z\text{ coprime},p\nmid s\right\}\subseteq\Q$.
\end{example}
\begin{enumerate}
    \item Let $P\subseteq A$ a prime ideal, $S=A\setminus P$.
        Then $A_p=S^{-1}A$ is a local ring with maximal ideal $PA_p=\left\{\frac{a}{s}:a\in P,s\notin P\right\}$.
        Then $S^{-1}A$ is an $A-$algebra with $\alpha:A\to S^{-1}A$ with $a\mapsto\frac{a}{1}$.
        Given an ideal $I\subseteq A$, $IS^{-1}A$ is the ideal in $S^{-1}A$ generated by $\alpha(I)$, the \textbf{extension ideal}.
        This is a \textbf{localisation of $A$ at $P$}/
    \item Given $f\in A\setminus\{0\}$, $S=\{1,f,f^2,\ldots,\}$ and $A_f:= S^{-1}A$.
        This is the \textbf{localisation of $A$ at $f$}.
        Note that $A_f$ is not necessarily a local ring.
        If $f$ is a unit, then $A_f\cong A$ via $\alpha$.
\end{enumerate}
...
Let $A$ be a ring, $S\subseteq A$ a multiplicatively closed set.
If $M$ is an $A-$module, we can define an $S^{-1}A-$module structure on $S^{-1}M$ as follows: on $M\times $S, define an equivalence relation $(x,s)\equiv(x',s')$ if there exists $t\in S$ such that $t(s'x-sx')=0$.
Denote $(x,y)/\equiv$ by $x/s$, and define $S^{-1}M:=M\times S/\equiv$ with operations
\begin{equation*}
    \frac{x}{s}+\frac{y}{t}=\frac{tx+sy}{st},\qquad\frac{a}{s}\cdot\frac{x}{t}=\frac{ax}{st}
\end{equation*}
\begin{example}
    If $A$ is an integral domain, $S=A\setminus\{0\}$, then $S^{-1}A=\Frac(A)=A_{(0)}$.
    Then $M$ is an $A-$module, so $S^{-1}M$ is a $\Frac(A)-$vector space.
\end{example}
\begin{proposition}
    $S^{-1}$ is an exact covariant functor on $A-$modules.
\end{proposition}
\begin{proof}
    $S^{-1}$ acts on $A-$linear maps as follows: $f:M\to N$ is an $A-$module homomorphism, $S^{-1}f:S^{-1}M\to S^{-1}N$ by $\frac{x}{s}\mapsto\frac{f(x)}{s}$.
    One must check:
    \begin{enumerate}[nolistsep]
        \item The map is well-defined.
        \item $S^{-1}A$-linear
        \item If $M\fto{f}N\fto{g}K$, then $S^{-1}(g\circ f)=S^{-1}g\circ S^{-1}f$, and $S^{-1}M\fto{S^{-1}f}S^{-1}N\fto{S^{-1}g}S^{-1}K$.
    \end{enumerate}
    To verify exactness, suppose $M\fto{f} N\fto{g} K$ is exact.
    Consider $S^{-1}M\fto{S^{-1}f}S^{-1}N\fto{S^{-1}g}S^{-1}K$.
    Since $\im(f)\subseteq\ker(g)$, $g\circ f=0$.
    Thus $0=S^{-1}(g\circ f)=S^{-1}(g)\circ S^{-1}(f)$, so $\im(S^{-1} f)\subseteq\ker(S^{-1}g)$.
    Conversely, if $\frac{m}{s}\in\ker(S^{-1}g)$ with $m\in N$, then $0=S^{-1}g(m/s)=\frac{g(m)}{s}$.
...
\begin{corollary}
    \begin{enumerate}[nolistsep]
        ...
        \item If $N\leq M$ is a submodule, then $S^{-1}(M/N)\cong S^{-1}M/S^{-1}N$.
...
\begin{proof}
    \begin{enumerate}[nolistsep]
        ...
        \item Since $0\to N\to M\to M/N\to 0$ is exact, so is $0\to S^{-1}M\to S^{-1}M\to S^{-1}(M/N)\to 0$ is exact, and $S^{-1}(M/N)\cong S^{-1}M/S^{-1}N$.
        \item One can check using the definition
        \item Exercise.
    \end{enumerate}
\end{proof}
\begin{proposition}
...
\begin{proof}
    ...
    \begin{align*}
        \sum_{i=1}^l\left(x_i\otimes\frac{a_i}{s_i}\right)&=\sum_{i=1}^l\left(x_i\otimes \frac{a_it_i}{t}\right)\\
                                                          &= \sum_{i=1}^l\left(a_it_ix_i\otimes\frac{1}{t}\right)=\left(\sum_{i=1}^l a_it_ix_i\right)\otimes\frac{1}{t}
    \end{align*}
    Now if $x\otimes\frac{a}{s}\in\ker(f)$, then $\frac{ax}{s}=0$ so get $t$ so $tax=0$.
    Thus $x\otimes\frac{a}{s}=tax\otimes\frac{1}{ts}=0$.

    Thus $f$ is an $A-$linear isomorphism, so we can check that it is $S^{-1}A-$linear.
\end{proof}
We actually have that tensoring and taking fractions are isomorphisms of functors.
\begin{corollary}
    $S^{-1}A$ is a flat $A-$algebra, i.e. $\otimes S^{-1}A$ is exact.
\end{corollary}
\begin{proof}
    Suppose $M'\to M\to M''$ is exact; since $S^{-1}$ is exact,
    \begin{center}
        \begin{tikzcd}
...
\end{example}
\begin{definition}
    If $B$ is an $A-$algebra, $I\subseteq A$ an ideal, then $IB$ is the ideal generated by $f(I)$, the \textbf{extension ideal}.
    If $J\subseteq B$ is an ideal, then $J\cap A:=f^{-1}(J)$ is the \textbf{contraction ideal}.
\end{definition}
\begin{proposition}
    ...
    \begin{enumerate}[nolistsep,label=(\roman*)]
        ...
        \item If $I\subseteq A$ is an ideal, then $A\cap (S^{-1}I)=\bigcup_{s\in S}\{x\in A:sx\in I\}$.
            We write $(I:s)=\{x\in A:sx\in I\}$.
        ...
        \item We have a bijective correspondence $\{P\in\Spec(A):P\cap S=\emptyset\}\longleftrightarrow\Spec(S^{-1}A$ with $P\mapsto S^{-1}P$ and $Q\cap A\mapsfrom Q$.
    \end{enumerate}
\end{proposition}
\begin{proof}
    \begin{enumerate}[label=(\roman*)]
...
\end{proof}
\begin{remark}
    If $I\subseteq A$ is an ideal, then $IS^{-1}A=S^{-1}I=\{a/s:a\in I\}$.
    If $a\in I$, then $a/s=a/1\cdot 1/s\in IS^{-1}A$.
    Conversely, one can use the tensor representation.
\end{remark}
