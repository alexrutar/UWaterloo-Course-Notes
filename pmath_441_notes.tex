...
\begin{corollary}
    ...
    Then
    \begin{equation*}
        \quot{\mathcal{O}_K}{I}\cong \Z_{a_{11}}\times\cdots\times\Z_{a_nn}
    \end{equation*}
\end{corollary}
\begin{proof}
    Consider the map
...
If $x\neq\lambda_i$ for all $i$, then $x$ has $n$ distinct preimages.
However, if $x=\lambda_i$, then $x$ has $n-e_i$ preimages so that $\lambda_i$ ramified.
...
If $[K:\Q]=n$, then for most prime ideals $(p)\subseteq\Z$, $(p)$ has $n$ preimages in this map.
We say that $P$ is ramified if and only if $\pi^{-1}(p)<n$.
\begin{theorem}
...
\end{proposition}
\begin{proof}
    Let $\alpha_1,\ldots,\alpha_{N(I)}$ be distinct representatives in $\quot{\mathcal{O}_K}{I}$.
    Then $1+\alpha_1,\ldots,1+\alpha_{N(I)}$ are also distinct representatives and thus a permutation of $\alpha_1,\ldots,\alpha_{N(I)}$.
    Thus in $\quot{\mathcal{O}_K}{I}$,
    \begin{equation*}
        N(I)+\sum_{i=1}^{N(I)}\alpha_i=\sum_{i=1}^{N(I)}\alpha_i
    \end{equation*}
    so that $N(I)=0$ in $\mathcal{O}_K/I$, i.e. $N(I)\in I$.
\end{proof}
\begin{remark}
