...
\begin{proof}
    ...
    Then if $\sigma_i$ are the embeddings $\Q(\alpha)\hookrightarrow\C$ fixing $\Q$, $\sigma_1=\id$,
    \begin{equation*}
        \pm 1 =\prod_{i=1}^n\sigma_i(\alpha)=\alpha\prod_{i=2}^n\sigma_i(\alpha)
    \end{equation*}
    Note that each $\sigma_i(\alpha)\in\mathcal{O}_{\overline{Q}}$, but since $\Q(\alpha)$ may not be normal, $\sigma_i(\alpha)$ may not be in $\Q(\alpha)$.
...
\begin{example}[Quadratic Number Field]
    ...
    \end{equation*}
\end{example}
\begin{proposition}\label{prop:algbasis}
...
\begin{proof}
    ...
    Let $\sigma_1,\ldots,\sigma_n:K\hookrightarrow\C$ extend $\Q\subseteq\C$.
