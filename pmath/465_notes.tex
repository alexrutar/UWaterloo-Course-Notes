...
\end{align*}
\begin{definition}
    A \textbf{smooth $k-$form} in $U\subseteq\R^n$ is an expression of the form $a(x)=\sum_Ia_I(x) dx^I$ where the sum is taken over multi-indices $I=(i_1,\ldots,i_k)$ with $1\leq i_1<\cdots<i_k\leq n$ and each $a_I:U\subseteq\R^n\to\R$ is a smooth map and $dx^I=dx^{i_1}\weddge\cdots\wedge dx^{i_k}$.
\end{definition}
...
When $\phi$ is a chart on $M$ at $p$ and $\alpha=\sum_Ia_Idx^I\in\Lambda^kT_p^*M$ and $\beta=\sum_Jb_Jdx^J\in\Lambda^\ellT_p^*M$, we would like to define $\alpha\wedge\beta\in\Lambda^{k+\ell}T_p^*M$ by $\alpha\wedge\beta=\sum_{I,J}a_Ib_Jdx^I\wedge dx^J$ (where we can use the rule $dx^i\wedge dx^j=-dx^j\wedge dx^i$ to put the multi-index in increasing order).
