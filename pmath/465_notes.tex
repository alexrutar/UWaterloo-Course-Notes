...
\end{definition}
Such a manifold with boundary can be given an atlas of charts $\phi_k:U_k\to\phi_k(U_k)$, $k\in K$, such that for each $k\in K$, either $\phi_k(U_k)\subseteq\R^n$ or $\phi_k(U_k)\subseteq\mathbb{H}^n$ with $\phi_k(U_k)\cap\partial\mathbb{H}^n=\emptyset$.
