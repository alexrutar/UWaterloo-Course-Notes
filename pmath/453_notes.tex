...
\begin{lemma}[Closest Approximation to Finite]
    ...
    \begin{enumerate}[nl,r]
        \item $P_{\mathcal{M}}x=\sum_{j=1}^n \inr{x,e_j}e_j$ satisfies that $x-P_{\mathcal{M}}x\per\mathcal{M}$ and hence $\norm{x}^2=\norm{P_{\mathcal{M}}}^2+\norm{x-P_{\mathcal{M}}x}^2$
        \item $\dist(x,\mathcal{M})=\norm{x-\sum_{j=1}^n(x,e_j)e_j}^{1/2}$
    \end{enumerate}
\end{lemma}
\begin{proof}
    \begin{enumerate}[nl,r]
...
\begin{proof}
    ...
    Inductively, $z_n\in\mathcal{E}_n$ and $z_n\per\mathcal{E}_k$ for $k=1,\ldots,n-1$.
