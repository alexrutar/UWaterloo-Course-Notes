...
\begin{equation*}
    \conv(S)=\bigcap_{\substack{S\subseteq C\subseteq X\\C\text{ convex}\right}}C
\end{equation*}
If $X$ is normed, we let $\co(S)$ denote the \textbf{closed convex hull}, i.e. the closure of the convex hull.
\begin{definition}
...
\begin{proof}
    ...
    Now, let $x_0\in D(X)$ be so that $|F(x_0)|>(1-\epsilon_\norm{F}$.
...
\begin{proof}
    ...
    Since $f_0$ is arbitrary, we have $\tau_\rho\subseteq w^*|_{B(X^*)}$, but since $B(X^*)$ is $w^*-$compac and $\tau_\rho$ is Hausdorff, these must be equal.
\end{proof}
\begin{enumerate}[nl,r]
