...
\begin{theorem}
    ...
    \begin{enumerate}[nl,r]
        ...
        \item \textit{(Axiom of Choice 2)}: Given any $\{A_\lambda\}_{\lambda\in\Lambda}$ where $A_\lambda\neq\emptyset$ for each $\lambda$,
            ...
            \end{equation*}
        \item \textit{(Zorn's Lemma)}: In a partially ordered set $(S,\leq)$, if each chain $C\subseteq S$ admits an upper bound in $S$, then $(S,\leq)$ admits a maximal element.
...
\begin{example}
    \begin{enumerate}[nl,r]
        ...
        \item \textit{Metric topology:} A metric space $(X,d)$ is naturally a topological space, where the topology is given by
            \begin{equation*}
            ...
            \end{equation*}
            Note that $d\sim\rho$ implies that $\tau_d=\tau_\rho$, but the reverse implication is not true: for example, the metrics on $\R$ given by $d(x,y)$ and $\rho(x,y)=\frac{|x-y|}{1+|x-y|}$ are not equivalent.
...
\end{proof}
An important class of topologies which are particularly tractable are the Hausdorff spaces:
\begin{definition}
    A topological space $(X,\tau)$ is \textbf{Hausdorff} if given $x\neq y$ in $X$, there are $U_x,V_y\in\tau$ such that $x\in U_x$ and $y\in V_y$ and $U_x\cap U_y=\emptyset$.
\end{definition}
\begin{example}
    \begin{enumerate}[nl,r]
...
\end{definition}
A basic application of the definitions yields the following:
\begin{proposition}
    ...
    \end{enumerate}
\end{proposition}
Interestingly, if the $\tau$ is a Hausdorff topology, we also have a converse for (i), which is again left as an interesting exercise:
\begin{proposition}
    Let $(X,\tau)$ be a Hausdorff space, $K\subseteq X$ $\tau-$compact.
    Then $K$ is $\tau-$closed.
\end{proposition}
This yields the following nice result for compact subsets of Hausdorff spaces.
\begin{proposition}
    Let $(X,\tau)$ be a compact space.
    \begin{enumerate}[nl,r]
...
\begin{proof}
    \begin{enumerate}[nl,r]
        \item If $F\subset X$ is closed, it is compact, so that $(\phi^{-1})^{-1}\phi(F)\subset Y$ is compact, and hence closed.
    ...
    \end{enumerate}
\end{proof}
Here we give an alternative way to characterize compactness in a topological space:
\begin{definition}
...
\end{proof}
\subsection{Bases and Initial Topologies}
\begin{definition}
    Let $(X,\tau)$ be a topological space.
...
\begin{equation*}
    \tau_\beta=\left\{\bigcup_{i\in I}B_i:\{B_i\}_{i\in I}\subset \beta\right\}
\end{equation*}
defines a topology.
\begin{definition}
    Let $X\neq\emptyset$.
    ...
    \begin{equation*}
        \beta=\left\{\bigcap_{k=1}^n f_{\alpha_k}^{-1}(U_{\alpha_k}),n\in\N,\alpha_1,\ldots,\alpha_n\in A,\text{ each }U_{\alpha_k}\in\tau_{\alpha_k}\right\}
    \end{equation*}
\end{definition}
...
\end{remark}
A number of common topologies can be defined using this notation:
\begin{example}
    \begin{enumerate}[nl,r]
    ...
    \end{enumerate}
\end{example}
\subsection{Ultrafilters and Tychonoff's Theorem}
\begin{definition}
...
\begin{proposition}[Inequalities in $\lp{p}-$spaces]
    ...
    \end{itemize}
\end{proposition}
In particular, Minkowski's Inequality \TODO{ref certain labels by name? and also link - would be nice}
...
An important application of Hahn-Banach is the Hyperplane Separation Theorem.
...
\end{definition}
\begin{proposition}\label{p:fker}
    Let $X$ be a normed space.
    \begin{enumerate}[nl,r]
...
\begin{proof}
    \begin{enumerate}[nl,r]
        \item To see (i), $\ker f=f^{-1}(\{0\})$ is a closed set since $f$ is continuous.
            ...
            To see this, since $Y$ is closed, if $Y$ is dense at $y_0$, get $\delta>0$ such that $y_0+\delta D(X)\subseteq Y$.
            Then $D(X)\subseteq\frac{1}{\delta}(Y-y_0)=Y$, so $X=\spn D(X)\subseteq Y$.

        \item Suppose that $\ker f$ is not dense in $X$.
            Then there would be $x_0\in X$ and $\delta>0$ such that $(x_0+\delta D(X))\cap \ker f=\emptyset$, so $0\notin f(x_0)+\delta f(D(X))$ and
            \begin{equation}\label{e:idx}
                \frac{1}{\delta}f(x_0)\notin -f(D(X))=f(D(X)).
            \end{equation}
           Then $\norm{f}\leq\frac{1}{\delta}f(x_0)$: to see this, suppose $\norm{f}>\frac{1}{\delta}f(x_0)$ and get $x\in D(X)$ such that $|f(x)|>\frac{1}{\delta}|f(x_0)|$.
           But then $\left\lvert\frac{1}{\delta}f(x_0)\right\rvert<c$ where $c\in f(D(X))$...\TODO{Fix??}
    \end{enumerate}
\end{proof}
\begin{definition}
    Let $\emptyset\neq A\subseteq X$.
    ...
    \begin{itemize}[nl]
        ...
        \item \textbf{absorbing} at $a_0\in A$ if for any $x\in X$, there is $\epsilon=\epsilon(a_0,x)>0$ such that $a_0+tx\in A$ for $0\leq t<\epsilon$.
    \end{itemize}
\end{definition}
For example, if $X$ is a normed space, then any open set is absorbing around any of its points.
\begin{lemma}[Minkowski Functional]\label{l:mink-f}
    Let $A\subset X$ be a convex set containing 0 and absorbing at 0.
    Then the map $\rho:X\to\R$ given by $\rho(x)=\inf\{t>0:x\in tA\}$ is a sublinear functional such that
    \begin{enumerate}[nl,r]
        \item $\{x\in X:\rho(x)<1\}\subseteq A\subseteq\{x\in X:\rho(x)\leq 1\}$ and
        \item if $X$ is normed and $A$ is a neighbourhood of $0$, then there is $N>0$ such that $\rho(x)\leq N\norm{x}$ for $x\in X$.
    \end{enumerate}
\end{lemma}
\begin{proof}
    Note that $\rho$ is well-defined since $A$ is absorbing at $0$.
    We verify that $\rho$ is a sublinear functional:
    \begin{itemize}[nl]
        \item \textit{(non-negative homogeneity):}
            Clearly $\rho(0)=0$.
            If $s>0$ and $x\in X$, then
            \begin{equation*}
                \rho(sx)=\inf\{t>0:sx\in tA\}=s\cdot\inf\left\{\frac{t}{s}>0:x\in\frac{t}{s}A\right\}=s\rho(x).
            \end{equation*}
        \item \textit{(subadditivity):}
            If $s,t>0$ and $a,b\in A$, then by convexity
            \begin{equation*}
                sa+tb=(s+t)\left(\frac{s}{s+t}a+\frac{t}{s+t}b\right)\in(s+t)A
            \end{equation*}
            so that $sA+tA\subseteq(s+t)A$.
            Clearly $(s+t)A\subseteq sA+tA$ so that $sA+tA=(s+t)A$.

            Now for $x,y\in X$, we have
            \begin{align*}
                \rho(x)+\rho(y) &= \inf\{s>0:x\in sA\}+\inf\{t>0:y\in tA\}\\
                                &= \inf\{s+t:s>0,t>0,x\in sA,y\in tA\}\\
                                &\geq\inf\{s+t:s>0,t>0,x+y\in sA+tA=(s+t)A\}\\
                                &= \inf\{r>0:x+y\in rA\}=\rho(x+y).
            \end{align*}
    \end{itemize}
    Now we see that it satisfies the required properties:
    \begin{enumerate}[nl,r]
        \item If $\rho(x)<1$, then there is $0<t<1$ so $\frac{1}{t}x\in A$ and since $0\in A$ and $A$ is convex $x=(1-t)\cdot0+t\cdot\frac{1}{t}x\in A$.
            The second inclusion is obvious.
        \item Get $\delta>0$ so $\delta D(X)\subseteq A$.
            Then for $x\in X$ and any $\epsilon>0$,
            \begin{equation*}
                x\in(\norm{x}+\epsilon)D(X)\subseteq\frac{\norm{x}+\epsilon}{\delta}A
            \end{equation*}
            so $\rho(x)\leq\frac{\norm{x}+\epsilon}{\delta}$; since $\epsilon>0$ was arbitrary, $\rho(x)\leq\frac{1}{\delta}\norm{x}$ so the result follows with $N=1/\delta$.
    \end{enumerate}
\end{proof}
\begin{theorem}[Hyperplane Separation]\label{t:hyp-sep}
    ...
    \begin{itemize}[nl]
        \item if $A$ is a neighbourhood of $a_0$, $f\in X^*$
        \item if $A$ is absorbing around each of its points, $\Re f(a)>\alpha\geq\Re f(b)$.
    \end{itemize}
\end{theorem}
\begin{proof}
    We first recenter at $0$.
    It is easy to verify that
    \begin{itemize}[nl]
        \item $A-B$ is absorbing at any $a_0-b$, $b\in B$
        \item $A-B$ is convex
        \item if $X$ is normed and $A$ a neighbourhood of $a_0$, then $A-B$ is a neighbourhood of each $a_0-b$, $b\in B$; and
        \item if $A$ is absorbing around any of its points, then $A-B$ is absorbing around any of its points.
    \end{itemize}
    Let $x_0=a_0-b_0$ for some $b_0\in B$ and set $C=x_0-(A-B)$; note that $0=x_0-x_0\in C$.
    Then by the above points, $C$ is absorbing at 0, convex, and if $X$ is normed and $A$ a neighbourhood of $a_0$, then $C$ is a neighbourhood of $0$; and if $A$ is absorbing at any of its points, then $C$ is absorbing at each of its points.

    Let $\rho$ be the Minkowski functional of $C$.
    Notice that since $A\cap B=\emptyset$, $0\notin A-B$ so $x_0\notin C$.
    Thus by (i) of \cref{l:mink-f}, $\rho(x_0)>1$.

    Let us find $f$ and $\alpha$.
    Let $f_0:\spn_{\R}\{x_0\}\to\R$, by $f_0(sx)=s\rho(x_0)$.
    Then $f_0$ is linear and $f_0\leq \rho|_{\spn_{\R}\{x_0\}}$, so by Hahn-Banach, get $f\in X_{\R}'$ such that $f\leq \rho$ on $X$.
    If $a\in A$ and $b\in B$, then $x_0-(a-b)\in C$ so that
    \begin{equation*}
        f(x_0-(a-b))\leq \rho(x_0-(a-b))\leq 1
    \end{equation*}
    again by (i) of \cref{l:mink-f}.
    Thus $f(x_0)+f(b)\leq 1+f(a)$ and since $f(x_0)=\rho(x_0)$, we have $f(b)\leq f(a)$.
    Thus there exists some $\alpha\in\R$ such that
    \begin{equation*}
        \sup\{f(b):b\in B\}\leq\alpha\leq\inf\{f(a):a\in A\}
    \end{equation*}
    If $\F=\R$, we are done; otherwise, replace $f$ by $f_{\C}$ and the result follows.

    For the remainder of the proof, we suppose $X$ is a normed space, and $A$ is a neighbourhood of $a_0$.
    Then (ii) of \cref{l:mink-f} provides $N>0$ so that $\rho(x)\leq N\norm{x}$.
    Then for $x\in X$, $f(x)\leq \rho(x)\leq N\norm{x}$ and $-f(x)=\rho(-x)\leq N\norm{-x}=N\norm{x}$ so $|f(x)|\leq N\norm{x}$ and $f\in X^*$.
    ...
    But then $\alpha\leq f(a-tx_0)=f(a)-tf(x_0)<\alpha$ since $f(x_0)=\rho(x_0)>0$, a contradiction.
\end{proof}
\begin{definition}
    If $\emptyset\neq S\subset X$, then its \textbf{convex hull} is given by
    \begin{equation*}
        \conv(S) = \left\{\sum_{i=1}^n\lambda_jx_j:n\in\N,x_j\in S,\lambda_j\geq 0,\sum_{j=1}^n\lambda_j=1\right\}
    \end{equation*}
\end{definition}
One can verify that $\conv(S)$ is in fact convex, and is the smallest convex set containing $S$, in other words that
\begin{equation*}
    \conv(S)=\bigcap_{\substack{S\subseteq C\subseteq X\\C\text{ convex}}}C
\end{equation*}
...
\end{definition}
If $X$ is normed, then \cref{p:fker} shows that $H$ is closed if and only if $f$ is bounded.
We now have the following consequence of \cref{t:hyp-sep}:
\begin{corollary}\label{c:cvxh}
    If $X$ is a normed vector space and $\emptyset\neq S\subset X$, then
    \begin{equation*}
        \overline{\conv}(S)=\bigcap_{\substack{S\subseteq H\subset X\\H\text{ closed half space}}}H.
    \end{equation*}
\end{corollary}
\begin{proof}
    The forward inclusion is trivial, so we assume $x_0\notin \overline{\conv}(S)$.
    Then there is $\delta>0$ such that $(x_0+\delta D(X))\cap\overline{\conv}(S)=\emptyset$.
    Since $x_0+\delta D(X)$ is open and convex, \cref{t:hyp-sep} gives provides $f\in X^*$ and $\alpha\in\R$ so $\Re f(a)>\alpha\geq\Re f(b)$ for $a\in x_0+\delta D(X)$ and $b\in\overline{\conv}(S)$.
    Then $S\subset H=\{y\in X:\Re f(x)\leq\alpha\}$ but $x_0\notin H$.
\end{proof}
\subsection{Banach-Steinhaus}
We now prove a straightforward but important corollary of the Baire Category Theorem:
\begin{theorem}[Banach-Steinhaus]
    Let $X,Y$ be normed spaces, $U\subseteq X$ be non-meagre, and $\mathcal{F}\subset\mathcal{B}(X,Y)$ be such that for each $x\in U$, $\sup\{\norm{Tx}:T\in\mathcal{F}\}<\infty$.
...
\end{proof}
\subsection{Open Mapping and Closed Graph}
...
\end{proof}
\begin{theorem}[Inverse Mapping]\label{t:imp}
...
\end{theorem}
\begin{proof}
    Let $T\in\mathcal{B}(X,Y)$ and suppose $(x,y)=\lim_{n\to\infty}(x_n,Tx_n)$ with $(x_n,Tx_n)\in\Gamma(T)$.
    Then
    \begin{align*}
        \norm{y-Tx}&\leq \norm{y-Tx_n}+\norm{Tx_n-Tx}\leq\norm{y-Tx_n}+\norm{T}\norm{x_n-x}\\
                   &\leq\max{\norm{T},1}\cdot\norm{(x,y)-(x_n,Tx_n)}_1
    \end{align*}
    so that $y=Tx$ and $(x,y)=(x,Tx)\in\Gamma(T)$.

    Conversely, if $\Gamma(T)$ is closed in $X\oplus_1 Y$, then $\Gamma(T)$ is a Banach space.
    Define $\pi:\Gamma(T)\to X$ by $\pi(x,Tx)=x$ (projection onto the first coordinate).
    Notice that $\pi$ is linear, and
    \begin{equation*}
        \norm{\pi(x,Tx)}=\norm{x}\leq\norm{(x,Tx)}_1
    \end{equation*}
    so $\norm{\pi}\leq 1$ and $\pi$ is bounded.
    Moreover, $\pi$ is a bijection where $\pi^{-1}:X\to\Gamma(T)$ is given by $\pi^{-1}(x)=(x,Tx)$, so by \cref{t:imp} $\pi^{-1}$ is bounded.
...
\end{enumerate}
\section{Geometry and Topology of Banach Spaces}
\subsection{Bounded Complementation}
...
\begin{theorem}
    ...
    Then $X$ admits a closed complement $Z$ if and only if there is some $P\in\mathcal{B}(Y)$ such that $P\circ P=P$ and $\im P=P(Y)=X$.
\end{theorem}
\begin{remark}
...
\end{proof}
\subsection{Characterization of Finite Dimensionality}
\begin{theorem}
...
\end{proof}
\subsection{On Compactness of the Unit Ball}
\begin{lemma}\label{l:ndist}
    Let $X$ be a normed space and $Y\subsetneq X$ a closed subspace.
    Then for any $0<\epsilon<1$ there is some $x_0\in D(X)$ such that $\dist(x_0,Y)>1-\epsilon$.
\end{lemma}
\begin{proof}
    Let $x\in X\setminus Y$ and $f:\spn(\{x\}\cup Y)\to\F$ be given by $f(y+\alpha x)=\alpha$ where $y\in Y$ and $\alpha\in \F$.
    In particular, since $\ker f=Y$ is a closed proper subspace of $\spn(\{x\}\cup Y)$, by \cref{p:fker}, $f$ is bounded.
    Thus apply Hahn-Banach to get $F\in X^*$ with $\norm{F}=\norm{f}$.

    Now, let $x_0\in D(X)$ be so that $|F(x_0)|>(1-\epsilon)\norm{F}$.
    Since $Y\subseteq\ker F$, for $y\in Y$,
    \begin{equation*}
        \norm{F}\norm{x_0-y}\geq|F(x_0-y)|=|F(x_0)|>(1-\epsilon)\norm{F}
    \end{equation*}
    so $\norm{x_0-y}>1-\epsilon$.
    But $y\in Y$ was arbitrary, so $\dist(x_0,Y)=\inf_{y\in Y}\norm{x_0-y}\geq 1-\epsilon$, as required.
\end{proof}
\begin{theorem}
    Let $X$ be a normed space.
...
\end{theorem}
\begin{proof}
    To prove the interesting direction, suppose $X$ is not finite dimensional.
    Let $\epsilon\in(0,1)$ and let $x_1\in B(X)\setminus\{0\}$.
    Inductively, for each $n\in\N$, given $\{x_1,\ldots,x_n\}$, by \cref{l:ndist} get $x_{n+1}\in B(X)$ so that $\dist(x_{n+1},\spn\{x_1,\ldots,x_n\})\geq 1-\epsilon$.
    But then by construction for $i\neq j$ $d(x_i,x_j)>1-\epsilon$ so $B(X)$ is not totally bounded, and hence not compact.
\end{proof}
\subsection{Weak and Weak* Topologies}
Let $X$ be a vector space and $Z\subseteq X'$ a subspace.
...
In partiular, if $X$ is a normed space, then the \textbf{weak topology} on $X$ is $\omega=\sigma(X,X^*)$.
...
Here we have a key feature of the $w^*-$topology on a normed space, which follows as a consequence to Tychonoff's Theorem:
\begin{theorem}[Alaoglu]\label{t:ala}
    Let $X$ be a normed space.
...
\begin{theorem}[Metrization]
    If $X$ is a separable normed space, then $B(X^*)$ is $w^*-$metrizable.
\end{theorem}
\begin{proof}
    ...
    Then if $f\in V$,
    \begin{align*}
        \rho(f,f_0) &= \sum_{k=1}^n\frac{|f(x_k)-f_0(x_k)|}{2^k}+\sum_{k=n+1}^\infty\frac{|f(x_k)-f_0(x_k)|}{2^k}<\epsilon
    \end{align*}
    so that $V$ is contained in a metric neighbourhood of $f_0$.
    Since $f_0$ is arbitrary, we have $\tau_\rho\subseteq w^*|_{B(X^*)}$, but since $B(X^*)$ is $w^*-$compact and $\tau_\rho$ is Hausdorff, these must be equal.
\end{proof}
\begin{enumerate}[nl,r]
...
\subsection{Weak and Weak* Topologies in Convexity}
...
\end{lemma}
\begin{proof}
    Define $T:X\to\F^n$ by $Tx=(f_1(x),\ldots,f_n(x))$ so $\ker T=\bigcap_{i=1}^n\ker f_i$.
    Let $R=\im T\subseteq\F^n$ and $g_0\in R'$ by $g_0(Tx)=f_0(x)$.
    Certainly $g_0$ is linear, and $g_0$ is well-defined since $\ker T\subseteq\ker f_0$.
    Let $g\in(\F^n)'$ such that $g|_{R}=g_0$; get $\alpha_i\in\F$ so that $g(y_1,\ldots,y_n)=\sum_{j=1}^n\alpha_jy_j$.
    Hence for $x\in X$,
    \begin{equation*}
        f_0(x)=g_0(Tx)=g(Tx)=g(f_1(x),\ldots,f_n(x))=\sum_{j=1}^n\alpha_jf_j(x)
    \end{equation*}
    so that $f_0=\sum_{j=1}^n\alpha_jf_j\in\spn\{f_1,\ldots,f_n\}$.
\end{proof}
We now show that we have an analogue of norm separation with respect to the $w^*-$topology:
\begin{theorem}[$w^*-$Separation]\label{t:ws-sep}
...
\end{proof}
A set of the form $H=\{f\in X^*:\Re f(x)\leq\alpha\}$ for some fixed $x\in X$ and $\alpha\in\R$ is called a real $w^*-$halfspace.
Note that any such space is $w^*-$closed and convex.
\begin{theorem}[$w^*-$Closed Convex Hull]\label{c:wscvxh}
    If $S\subset X^*$ and $\mathcal{H}$ is the set of real $w^*-$halfspaces in $X$, then
    \begin{equation*}
        \cwx S=\bigcap_{\substack{H\in\mathcal{H}\\H\supseteq S}}H.
    \end{equation*}
\end{theorem}
\begin{proof}
    Firstly, $\bigcap_{H\in\mathcal{H},H\supseteq S}H$ is $w^*-$closed and convex since it is the intersection of such spaces.
    Conversely, if $f\in X^*\setminus\cwx S$, then there is a basic $w^*-$open neighbourhood
    \begin{equation*}
        B=\bigcap_{j=1}^n\hat x_j^{-1}(V_j)\subseteq X^*\setminus\cwx S
    \end{equation*}
    so that $B\cap\cwx S=\emptyset$.
    Then since $B$ is convex, apply \cref{t:ws-sep} to get a half space not containing $B$.
\end{proof}
The case for weak-closed spaces is easier, and follows from our earlier results on the norm-closed convex hull.
\begin{corollary}\label{c:wcvxh}
    If $C\subset X$ is convex, then $C$ is norm closed if and only if $C$ is $w-$closed.
\end{corollary}
\begin{proof}
    If $C$ is $w-$closed then it is automatically norm-closed.

    Conversely, suppose we are given a normed space $X$ and closed half space $H=\{x\in X:\Re f(x)\leq\alpha\}$ for some $f$ in $X^*$, $\alpha\in\R$.
    Then
    \begin{equation*}
        H=(\Re f)^{-1}([\alpha,\infty))=f^{-1}(\{z\in\C:\Re z\geq\alpha\})
    \end{equation*}
    is $w-$closed as well.
    Since $C$ is convex and norm-closed, by \cref{c:cvxh}, $C$ is the intersection of such half spaces, which are $w-$closed, so that $C$ is $w-$closed.
\end{proof}
\begin{definition}
    Let $X$ be a normed space.
    If $E\subseteq X$, the \textbf{polar} of $E$ is given by
    \begin{align*}
        E^\circ &= \bigcap_{x\in E}\{f\in X^*:\Re f(x)\leq 1\}.
    \end{align*}
    Conversely, if $F\subseteq X^*$, the \textbf{pre-polar} of $F$ is given by
    \begin{equation*}
        F_\circ=\bigcap_{f\in F}\{x\in X:\Re f(x)\leq 1\}.
    \end{equation*}
\end{definition}
Note that $E^\circ$ is convex, $w^*$-closed, and contains $0$.
Similarly, $F_0$ is convex, $w-$closed, and contains $0$.
\begin{theorem}[Bi-polar]\label{t:bip}
    \begin{enumerate}[nl,r]
...
\begin{proof}
    \begin{enumerate}[nl,r]
        \item Since $E\cup\{0\}\subseteq (E^\circ)_\circ$, by \cref{c:wcvxh}, $\cw(E\cup\{0\})\subseteq(E^\circ)_\circ$.
            Conversely, if $x_0\in X\setminus\cw(E\cup\{0\})$, then \cref{c:cvxh} provides some $f\in X^*$ and $\alpha\in \R$ so that $\Re f(x_0)>\alpha\geq\Re f(x)$ for $x\in E\cup\{0\}$.
            Notice that $\Re f(x_0)>\alpha\geq \Re f(0)=0$, so we may set
            \begin{equation*}
                \beta:=\frac{1}{2}(\Re f(x_0)+\alpha)
            \end{equation*}
            with $\beta>0$.
            Then $\Re f(x_0)>\beta\geq\Re f(x)$ for any $x\in E\cup\{0\}$.
            Let $g=\frac{1}{\beta}f$ so that for any $x\in E$, $\Re g(x)=\frac{1}{\beta}\Re f(x)\leq 1$ and $g\in E^\circ$, but $\Re g(x_0)>1$ so $x_0\notin (E^\circ)_\circ$.
        \item The proof is similar to above; use \cref{c:wscvxh}.
    \end{enumerate}
\end{proof}
Suppose $Y\subseteq X$ is a subspace.
If $f\in Y^0$ such that $\Re f(y)\leq 1$ for $y\in Y$, then $f(y)=0$ for all $y\in Y$.
In particular, we define $Y^a=\{f\in X^*:f|_Y=0\}$ to be the \textbf{annhilator} of $Y$, where $Y^a=Y^0$.
Likewise, if $Z\subseteq X^*$ is a subspace, then we define $Z_a=\{x\in X:f(x)=0\text{ for each }f\in Z\}$ to be the \textbf{pre-annhilator}, where $Z_a=Z_0$.
Notice that $Y^a$ and $Z_a$ are subspaces.

Since subspaces are closed and contain 0, we immediately have the following:
\begin{corollary}
    \begin{enumerate}[nl,r]
        \item If $Y\subseteq X$ is a subspace, then $(X^a)_a=\overline{X}$.
        \item If $Z\subseteq X^*$ is a subspace, then $(Z_a)^a=\overline{Z}^{w^*}$.
    \end{enumerate}
\end{corollary}
In the context of \cref{c:wcvxh}, if $S\subset X^*$ is norm-closed, it may not be $w^*-$closed.
\TODO{example??}
However, this relationship does hold in the special case when $S=B(X^*)$; and in fact, as a consequence of the Bi-polar theorem, we have a bit more:
\begin{lemma}\label{l:bxn}
    If $X$ is a normed space, then $B(X)^\circ=B(X^*)$ and $B(X^*)_\circ=B(X)$.
\end{lemma}
\begin{proof}
    If $f\in B(X)^\circ$, then $\Re f(x)\leq 1$ for any $x\in B(X)$.
    Thus for $x\in B(X)$,
    \begin{equation*}
        |f(x)|=\overline{\sgn f(x)}f(x)=f(\overline{\sgn f(x)}x)\leq 1,
    \end{equation*}
    so $\norm{f}\leq 1$ and $f\in B(X^*)$.
    Conversely, if $f\in B(X^*)$ and $x\in B(X)$, then $\Re f(x)\leq|f(x)|\leq 1$ so $f\in B(X)^\circ$.

    For the second equality, by \cref{t:bip},
    \begin{equation*}
        B(X^*)_\circ=(B(X)^\circ)_\circ=\cw{B(X)}=B(X).
    \end{equation*}
\end{proof}
\begin{theorem}[Goldstine]\label{t:gold}
    If $X$ is a normed space, then $B(\hat X)$ is weak*-dense in $B(X^{**})$.
\end{theorem}
\begin{proof}
    Note that, by definition, $B(X)^\circ=B(\hat X)_\circ$.
    Thus by \cref{t:bip} and \cref{l:bxn}, we have
    \begin{align*}
        \overline{B(\hat X)}^{w^*}&=\cwx B(\hat X)=(B(\hat X)_\circ)^\circ= (B(X)^\circ)^\circ = B(X^*)^\circ= B(X^{**})
    \end{align*}
    as required.
\end{proof}
\begin{remark}
    As an exercise, one can show that the conclusion of the theorem is not true for the norm topology by considering $\csz$ with bidual $\ell^\infty$.
\end{remark}
\begin{example}
    \begin{enumerate}[nl,r]
...
\end{definition}
Notice that $X^{**}=(X^*)^*$ is complete, and the bijection $\iota$ given by $\iota(x)=\hat x$ is an isometry, so a reflexive space is always complete.
Moreover,
\begin{align*}
    U = \bigcap_{i=1}^n f_i^{-1}(V_i) &\iff \iota(U)=\bigcap_{i=1}^n\hat{f_i}^{-1}(V_i)
\end{align*}
where $\hat{f_i}\in \widehat{X^*}\subseteq X^{***}$ is given by $\hat{f_i}(\hat{x})=f_i(x)$, so that $\iota$ is a $w-w^*|_{\hat{X}}-$homeomorphism.
\begin{theorem}
...
\begin{proof}
    ...
    Since $B(X^{**})$ is $w^*-$compact by \cref{t:ala}, $\iota^{-1}(B(X^{**}))=B(X)$ is $w-$compact.

    \imp{ii}{i}
    If $B(X)$ is $w-$compact, by continuity of $\iota$, $B(\hat{X})$ is $w^*-$compact (and therefore $w^*-$closed), so by \cref{t:gold}, $B(\hat{X})=B(X^{**})$ and $\hat{X}=X^{**}$ so $X$ is reflexive.

    \imp{i}{iii}
    We have $w=\sigma(X^*,X^{**})=\sigma(X^*,\hat X)=w^*$.

    \imp{iii}{iv}
    Since $B(X^*)$ is $w^*-$compact, it is $w-$compact and by the same proof as (ii) implies (i) applied to $X^*$, we have that $X^*$ is reflexive.

    \imp{iv}{i}
    Since $B(\hat X)\subset X^{**}$ is norm-closed, by \cref{c:wcvxh}, $B(\hat X)$ is $w-$closed, where $w=\sigma(X^{**},X^{***})=\sigma(X^{**},\widehat{X^*})=w^*$, so $B(\hat X)$ is $w^*-$closed.
    Thus by \cref{t:gold}, $B(X^{**})=B(\hat X)$, as required.
\end{proof}
\begin{corollary}
    \begin{enumerate}[nl,r]
...
\begin{definition}
    ...
    \end{enumerate}
    \textbf{positive} if in addition
    \begin{enumerate}[nl,r]
        \item[(iii)]$\lbr{x,x}\geq 0$ for all $x\in X$
    \end{enumerate}
    and \textbf{non-degenerate} if
    \begin{enumerate}[nl,r]
        \item[(iii')] $\lbr{x,y}=0$ for all $y\in X$ implies $x=0$.
    \end{enumerate}
\end{definition}
Note that positive Hermitian forms naturally induce a seminorm by $\rho(x)=\lbr{x,x}^{1/2}$.
In particular, if this form is also non-degenerate, then $\rho$ is a norm.
We summarize this result, along with some additional properties, in the following proposition:
\begin{proposition}
    Let $\lbr{\cdot,\cdot}$ be a positive Hermitian form and $\rho(x)=\lbr{x,x}^{1/2}\in[0,\infty)$.
    Then for $x,y\in X$ and $\alpha\in\F$, we have
    \begin{enumerate}[nl,r]
        \item $\rho(\alpha x)=|\alpha|\rho(x)$
        \item $|\lbr{x,y}|\leq \rho(x)\rho(y)$
        \item $\rho(x+y)\leq \rho(x)+\rho(y)$
        \item $\lbr{\cdot,\cdot}$ is non-degenerate if and only if $\lbr{x,x}>0$ for $x\in X\setminus\{0\}$.
    \end{enumerate}
    Furthermore, in the non-degenerate case,
    \begin{itemize}[nl]
        \item Equality in (ii) occurs if and only if $x,y$ are linearly dependent
        \item $\lbr{x,y}=\rho(x)\rho(y)$ if and only if there is $s\geq 0$ such that $x=sy$ or $y=sx$ if and only if equality holds in $(iii)$.
    \end{itemize}
\end{proposition}
\begin{proof}
    \begin{enumerate}[nl,r]
        \item $\rho(\alpha x)=(\alpha\overline{\alpha}\lbr{x,x})^{1,2}=|\alpha|\rho(x)$
        \item If $\alpha\in F$, then
            \begin{align*}
                0\leq \lbr{x-\alpha y,x-\alpha y}&=\lbr{x,x}-\overline{\alpha}\lbr{x,y}-\overline{\overline{\alpha}\lbr{x,y}}+|\alpha|^2\lbr{y,y}\\
                                                 &= \rho(x)^2-2\Re\overline{\alpha}\lbr{x,y}+|\alpha|\rho(y)^2
            \end{align*}
            Set $\alpha=\sgn\lbr{x,y}$ so that $\overline{\alpha}\lbr{x,y}=|\lbr{x,y}|$ so
            \begin{equation*}
                |\lbr{x,y}|\leq\frac{1}{2}\left(\rho(x)^2+\rho(y)^2\right)
            \end{equation*}
            Then if $t>0$, by (i),
            \begin{equation*}
                |\lbr{x,y}|=\left\lvert\lbr{tx,\frac{1}{t}y}\right\rvert\leq\frac{1}{2}(t^2p(x)^2+\frac{1}{t^2}p(y)^2).
            \end{equation*}
            If $\rho(x)=0$, we let $t\to\infty$ so that $\lbr{x,y}=0$; if $\rho(y)=0$, we let $t\to 0^+$ and again that $\lbr{x,y}=0$.
            If $\lbr{x,y}\neq 0$, set $t=\rho(y)/\rho(x)$ and we are done.
        \item By direct computation, we have
            \begin{align*}
                p(x+y)^2&=\lbr{x+y,x+y}=p(x)^2+2\Re[x,y]+p(y)^2\\
                        &\leq p(x)^2+2|\lbr{x,y}|+p(y)^2\\
                        &\leq p(x)^2+2p(x)p(y)+p(y)^2=(p(x)+p(y))^2
            \end{align*}
        \item By (ii), if $\rho(x)^2=\lbr{x,x}=0$, then $\lbr{x,y}=0$ for all $y$.
...
\begin{theorem}[Orthonormal Basis]
    Let $(\mathcal{E},\inr{\cdot,\cdot})$ be a Euclidean space, $E\subset\mathcal{E}$ an orthonormal set.
    Then the following are equivalent:
    \begin{enumerate}[nl,r]
...
\begin{proof}
    \begin{enumerate}[nl,r]
        \item We have $\mathcal{M}\subseteq\mathcal{M}^{\perp\perp}$ and $\mathcal{M}$ is complete and thus admis an orthogonal projection $P_{\mathcal{M}}H\to H$ with $\im P_{\mathcal{M}}=\mathcal{M}$ and $\im(I-P_{\mathcal{M}})=M^\perp$.
            Now if $x\in\mathcal{M}^{\perp\perp}$, $P_{\mathcal{M}}x\in\mathcal{M}$ so that $x-P_{\mathcal{M}}x\in\mathcal{M}^{\perp\perp}+\mathcal{M}=\mathcal{M}^{\perp\perp}$ so that $x-P_{\mathcal{M}}x\in\mathcal{M}^\perp$.
            Thus
            \begin{equation*}
...
\begin{proof}
    If $f=0$, let $x_0=0$.
    If $f\neq 0$, $\ker f\subsetneq H$ so $(\ker f)^{\perp\perp}=\ker f$, so $(\ker f)^\perp\neq\{0\}$.
...
\end{example}
\subsection{Spectral Theory for Bounded operators}
...
\begin{example}
    \begin{enumerate}[nl,r]
        ...
        \item Let $1\leq p<\infty$ and define $S:\ell_p\to\ell_p$ by $S(x_1,x_2,\ldots)=(0,x_1,x_2,\ldots)$.
            Notice that $S$ is linear and $\norm{Sx}_p=\norm{x}_p$, so $\norm{S}=1$; as well, $\ker S=\{0\}$ so that $0\notin\sigma_p(S)$.
            Suppose for contradiction $0\neq\lambda\in\sigma_p(S)$, so get some $0\neq x$ so that $Sx=\lambda x$.
            Let $k$ be such that $x_k\neq 0$; but then $0=(S^kx)_k=\lambda^kx_k\neq 0$, a contradiction, so that $\sigma_p(S)=\emptyset$.
    \end{enumerate}
\end{example}
...
\begin{remark}
    \begin{enumerate}[nl,r]
        ...
        \item We have $[\im(\lambda I-T)]^a=\ker(\lambda I-T^*)$ by kernel-annhilator so $\overline{\im}(\lambda I-T)=\ker(\lambda I-T^*)$ by annhilator-preannhilator, so that $\sigma_{com}(T)=\sigma_p(T^*)$.
    \end{enumerate}
\end{remark}
\begin{lemma}
...
\subsection{Spectral Theory for Compact Operators}
\begin{lemma}
...
\begin{remark}
    ...
    \end{equation*}
    so that $0\sum_{\substack{j=1\\j\neq j_0}}^n\alpha_j'(\lambda_j-\lambda_{j_0})x_j$, a contradiction.
\end{remark}
...
\begin{proof}
    We have
    \begin{align*}
        \norm{T}^2 &= \sup_{x\in B(\mathcal{H})}\norm{Tx}^2 = \sup_{x\in B(\mathcal{H})}\inr{Tx,Tx}\\
                   &= \sup_{x\in B(\mathcal{H})}\inr{T^*Tx,x} \leq\sup_{x\in B(\mathcal{H})}\norm{T^*Tx}=\norm{T^*T}
    \end{align*}
    while $\norm{T^*T}\leq\norm{T^*}\norm{T}=\norm{T}^2$.
\end{proof}
\begin{proposition}
...
\begin{proof}
    ...
    Hence if $k\in\N$, we use the $C^*-$identity and the fact that $N^*N$ is Hermitian to get
    \begin{equation*}
        \norm{N}^{2^{k+1}}=\norm{N^*N}^{2^k}=\norm{(N^*N)^{2^k}}=\norm{(N^{2^k})^*N^{2^k}}=\norm{N^{2^k}}^2
    \end{equation*}
    and hence inductively $\norm{N}^{2^k}=\norm{N^{2^k}}$.
    Now, by the spectral radius formula,
    \begin{equation*}
...
\end{proposition}
\begin{proof}
    If $\lambda\in\sigma_{ap}(H)$, then there is $(x_n)_{n=1}^\infty\subset S(\mathcal{H})$ such that $(\lambda I-H)x_n\to 0$.
    Then
    \begin{equation*}
        \lambda-\inr{Hx_n,x_n}=\lambda\inr{x_n,x_n}-\inr{Hx_n,x_n}=\inr{(\lambda I-H)x_n,x_n}\to 0
    \end{equation*}
    where $\inr{Hx_n,x_n}=\overline{\inr{x_n,hx_n}}=\overline{\inr{Hx_n,x_n}}\in\R$ for each $n$, so $\lambda\in\R$.
    Now $\sigma_{com}(H)=\sigma_p(H^*)=\sigma_p(H)\subseteq\sigma_{ap}(H)$, so that $\sigma(H)=\sigma_{ap}(H)\cup\sigma_{\com}(H)=\sigma_{ap}(H)$ as required.
\end{proof}
\begin{remark}
    \begin{enumerate}[nl,r]
    ...
    \end{enumerate}
\end{remark}
\begin{theorem}[Continuous Functional Calculus for Bounded Hermitian Operators]
...
\end{theorem}
\begin{proof}
    Let $\Pol(\sigma(H))$ denote the set of polynomial functions restricted to $\sigma(H)$.
    Then we have
    \begin{enumerate}[nl,r]
        \item $\sigma(p(H))=p(\sigma(H))$ by the Spectral Mapping Theorem
        \item $\norm{p(H)}=r(p(H))$ since $p(H)$ is normal.
    \end{enumerate}
    Let $\Phi_0:\Pol(\sigma(H))\to\mathcal{B}(\mathcal{H})$ by $\Phi_0(p)=p(H)$.
    ...
    Moreover, by (i) and (ii),
    \begin{equation*}
        \norm{p}_\infty=r(p(H))=\norm{p(H)}
    \end{equation*}
    and $\Phi_0:\Pol(\sigma(H))\to\mathcal{B}(\mathcal{H})$ is an isometry.

    Now, since $\sigma(H)$ is compact and $\sigma(H)\subseteq\R$, $\sigma(H)$ is conjugacy closed so by Stone-Weierstrass $\overline{\Pol(\sigma(H))}=C(\sigma(H))$, so $\Phi_0$ admits a unique extension $\Phi:C(\sigma(H))\to\mathcal{B}(\mathcal{H})$.
    \TODO{Explain exact statement of Stone-Weierstrass}
    Certainly $\Phi$ is an isometry; let's verify the non-trivial required properties.

    If $f,g\in C(\sigma(H))$, say $f=\lim p_n$, $g=\lim q_n$, then $\norm{fg-p_nq_n}_\infty\to 0$, and hence
    \begin{equation*}
        \norm{\Phi(fg)-\Phi(f)\Phi(g)}\leq\norm{\Phi(fg)-\Phi_0(p_nq_n)}+\norm{\Phi_0(p_n)\Phi_0(q_n)-\Phi(f)\Phi(g)}
    \end{equation*}
    which converges to $0$, so $\Phi(fg)=\Phi(f)\Phi(g)$.
    The remaining properties follow identically.
\end{proof}
...
\end{theorem}
\begin{proof}
    We have $\sigma(H)=\{\lambda_1,\lambda_2,\ldots\}\cup\{0\}$ with $\lim_{n\to\infty}\lambda_n=0$, so for each $n\in\N$, we may define $\phi_n=\chi_{\{\lambda_n\}}\in C(\sigma(H))$ and $\iota=\sum_{n=1}^\infty\lambda_n\phi_n$, where the infinite sum converges uniformly.
    By the continuous functional calculus, let $P_n=\phi_n(H)$, so that
