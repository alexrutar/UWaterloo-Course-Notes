...
Given a net $(x_v)_{v\in N}$ and an eventually cofinal $\phi:M\to N$, we call $(x_{\phi(\mu)})_{\mu\in M}$.
\begin{definition}
...
\begin{example}
    \begin{enumerate}[nl,r]
        ...
        \item (nets from filtering families).
...
\begin{definition}
    Let $(x_v)_{v\in N}$ be a net in $X$.
    We say that $x_0\in X$ is
    \begin{itemize}[nl]
        \item a \textbf{limit point} if for any $U\in\tau$ with $x_0\in U$, $(x_v)_{v\in N}$ is eventually in $U$.
            That is, there is $\nu_U$ such that $x_\nu\in U$ whenever $\nu\geq\nu_U$.
            We write $x_0=\lim_{v\in N}x_v$, the $\tau-$limit of $(x_v)_{v\in N}$.
        \item a \textbf{cluster point} of $(x_v)_{v\in N}$ if for any $U\in\tau$ with $x_0\in U$, $(x_v)_{v\in N}$ is frequently in $U$.
    \end{itemize}
\end{definition}
\begin{proposition}
...
\end{proposition}
\begin{proof}
    $(\Rightarrow)$ Let for each $v\in N$ and $U\in\tau$ with $x_0\in U$
    \begin{equation*}
        F_{v,U}=\{v'\in N:v'\geq v,x_{v'}\in U\}\neq\emptyset
    \end{equation*}
    We let $\mathcal{F}=\{F_{v,U}:v\in N,U\in\tau\text{ with }x_0\in U\}\subset\mathcal{P}(N)$, which is filtering.
...
\end{theorem}
\begin{proof}
    The map $x\mapsto\hat x$ is a $w-w^*|_{\hat X}-$homeomorphism.
