...
We let $\mathcal{L}(X,Y)$ denote the set of all linear maps from $X$ to $Y$, which is itself a vector space.
...
\begin{proof}
    ...
    \end{equation*}
\end{proof}
\begin{corollary}[Hahn-Banach, Alternative Forms]\label{c:hahn-ext}
...
\begin{proof}
    We have:
    \begin{enumerate}[nl,r]
        \item Define $\rho:X\to\R$ be given by $p(x)=\norm{f}\cdot\norm{x}$, so $p$ is sublinear and $\Re f\leq p|_Y$.
            Apply Hahn-banach to to this data and get $\tilde F\in X_{\R}^*$ such that $\tilde F|_Y=\Re f$ and $\tilde F\leq p$.
            Set $F=\tilde F_{\C}$.
...
\begin{itemize}[nl]
    ...
    \item $\im UP_n\perp\im UP_m$ for $n\neq m$.
        ...
        \end{align*}
\end{itemize}
For example, compute $U$, $|K|$ for $x\in\ell_2$ and $Kx=(0,x_1,\frac{1}{2}x_2,\frac{1}{3}x_3)$.
