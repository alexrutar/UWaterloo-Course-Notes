...
\newcommand{\lbr}[1]{\ensuremath{\langle#1\rangle}}
...
\begin{enumerate}[nl,r]
    ...
    \item The definition of $\rho$ above extends to all of $X^*\times X^*$.
        However, $X^*$ with the weak$\phantomP{}^*$ topology is not in metrizable if $X$ is infinite dimensional.
...
\begin{proof}
    ...
    Then for $i=1,\ldots,n$, $\hat x_i(f_0+Y)=\{f_0(x_i)\}\subset f_0(x_i)+\mathbb{D}$, so that $f_0+Y\subseeq U\subseteq B$.
    ...
    The next lemma shows that $F\in\spn\{\hat x_1,\ldots,\had x_n\}\subseteq\hat X$, i.e. $F=\hat x$ for some $x\in X$.
\end{proof}
\begin{lemma}
...
\begin{proof}
    ...
    Recall $w^*=\sigma(X^{**},\hat X^*)$, and $w^*|_{\hat X}=\sigma(\hatX,(\hat X)^*|_{\hat X})$ and we have for $x_0\in X$, net $(x_\nu)_{\nu\in N}$ in $X$,
    \begin{align*}
...
\begin{proof}
    ...
    \end{equation*}
    since each $f_{x_i}\in\ker\delat$, which is absurd.
    ...
    \begin{equation*}
        t|\nu(f)|\leqt\nu(|f|)\leq\hat x(|f|)=|f(x)|
    \end{equation*}
    so $\ker\nu\supseteq\her\hat x$ and as above, $\nu=\hat x$.
    Then $\mu=\hat x$.
\end{proof}
\begin{remark}
    For $\F=\R$ or $\F=\C$, it is similar to show that $\ext B(C^{\F}(X,\tau)^*)=\{z\hat x:z\in\F,|z|=1,x\in X^*\}$.
\end{remark}
Let $PA(\N)=\{\mu\in\FA(\N):\norm{\mu}_{\var}\leq 1,\mu(\N)=1\}$ so, as above, $PA(\N)$ is a $w^*=\sigma(\FA(\N),\ell_\infty)-$compact set.
\begin{proposition}
...
\begin{proposition}
    ...
    \begin{itemize}[nl]
        ...
        \item $\br{x,y}=p(x)p(y)$ if and only if there is $s\geq 0$ such that $x=sy$ or $y=sx$ if and only if equality holds in $(iii)$.
    \end{itemize}
\end{proposition}
\begin{proof}
    \begin{enumerate}[nl,r]
...
\begin{definition}
    ...
    If, further, $\mathcal{E}$ is complete with respect to the induced norm $\norm{x}=\lbr{x,x}^{1/2}$, then we call $(\mathcal{E},\lbr{\cdot,\cdot})$ a \textbf{Hilbert space}.
\end{definition}
\begin{examples}
    \begin{enumerate}[r]
        \item (Euclidean Space) $(C[0,1],\langle\cdot,\cdot\rangle)$ given by $\lbr{f,g}=\int_0^1 f\overline{g}$
        ...
        \item (Hilbert Space) $(L_2[0,1],\lbr{\cdot,\cdot})$, $\lbr{f,g}=\int_{[0,1]}f\overline{g}$.
        \item (Hilbert Space) $(\ell_2,\lbr{\cdot,\cdot})$, $\lbr{x,y}=\sum_{j=1}^\infty x_j\overline{y}_j$ (convergence by Hölder's inequality)
        \item (Non-separable Hilbert Space) Let $\Gamma$ be an uncountable set.
            ...
            Let $\Gamma_n=\{\gamma\in\Famma:a_\gamma\geq 1/n\}$ and we have
            \begin{equation*}
            ...
            Write $\lbr{x,y}=\sum_{\gamma\in\Gamma}x_\gamma\overline{y_\gamma}=\sum_{k=1}^\infty x_{\gamma_k}\overline{y_{\gamma_k}}$.
