...
\begin{example}
    ...
    Here is a good way to understand the regular rep of $G$.
\end{example}
Consider $X=\{v_g:g\in G\}$ be a set of symbols, and let $V=\Free(X)$.
Then define $\rho:G\to\GL(V)$ given by $\rho(g)(v_h)=v_{gh}$ for all $g,h\in G$.
\begin{example}
